defmodule Distributor do
  ...
  def handle_call({:add_to_complete_list, elevator, ip}, _from, complete_list) do
    IO.puts("New node in cluster -- elevatorip #{inspect elevator} and ip #{inspect elevator}")
    case {Enum.member?(Enum.map(complete_list, fn list_elevator -> list_elevator.ip end), elevator.ip), elevator.state != nil, ip == Node.self()} do
      {true, true, false} ->
        IO.puts("TRUE")
